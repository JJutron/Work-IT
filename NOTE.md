...
- 홈 기능 스테이지는 가짜 전표가 아니라 실제 계산기·장해등급·판례·신청 화면 캡처 루프다. 다시 찍을 때 `PYTHONPATH=/app python scripts/export_feature_html.py` → `node scripts/capture_feature_stages.mjs` → `python3 scripts/frames_to_gif.py`. 장해등급 GIF는 `feature-disability.gif`. Docker는 사용자가 띄운 뒤 캡처.
